...
TEXT ·mulP751(SB), $96-24
	...
	// [R8-R14,BX,AX,DX,DI,(SP)] <- (AH+AL)*(BH+BL)-AL*BL
	MOVQ	(SP), R8
	SUBQ	(CX), R8
	MOVQ	(8)(SP), R9
	SBBQ	(8)(CX), R9
	...
	// [R8-R14,BX,AX,DX,DI,(SP)] <- (AH+AL)*(BH+BL) - AL*BL - AH*BH
	MOVQ	(96)(CX), SI
	SUBQ	SI, R8
	MOVQ	(104)(CX), SI
	SBBQ	SI, R9
...
// are available.
#define mul256x448bmi2adx(M0, C, T0, T1, T2, T3, T4, T5, T6, T7, T8, T9) \
	MOVQ	0+M0, DX		\
	MOVQ    P751P1_5, AX    \
	MULXQ	AX, T1, T0      \
	MOVQ    P751P1_6, AX    \
...
// available.
#define mul256x448bmi2(M0, C, T0, T1, T2, T3, T4, T5, T6, T7, T8, T9) \
	MOVQ	0+M0, DX		\
	MOVQ    P751P1_5, AX    \
	MULXQ	AX, T1, T0   	\
	MOVQ    P751P1_6, AX    \
...
#define REDC(C, M0, MULS) 	\
    ...
    MULS(M0, 48+C, R8, R9, R13, R10, R14, R12, R11, BP, BX, CX) \
    XORQ    CX, CX          \
    MOVQ    48+C, AX        \
    MOVQ    56+C, DX        \
    MOVQ    64+C, BX        \
    ...
    MULS(32+M0, 48+C, R8, R9, R13, R10, R14, R12, R11, BP, BX, CX) \
    XORQ    CX, CX          \
    MOVQ    48+C, AX        \
    MOVQ    56+C, DX        \
    MOVQ    64+C, BX        \
    ...
    MULS(64+M0, 48+C, R8, R9, R13, R10, R14, R12, R11, BP, BX, CX) \
