...
The scheme requires p and q to be safe primes to provide verifiability.
So it is possible to validate (and reject) signature shares produced by malicious signers.
Note that RSA keys generated by the Go standard library are not guaranteed to be safe primes.
